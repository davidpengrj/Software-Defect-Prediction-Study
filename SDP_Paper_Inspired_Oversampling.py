...
try:
    ...
    print("提示: 未检测到 'imbalanced-learn'。")

warnings.filterwarnings('ignore')

# --- 配置保存路径 ---
MODEL_DIR = 'saved_models_oversampling'  # 实验组模型保存位置
...
# --- 步骤 2: 数据预处理 (过采样) ---
def preprocess_and_oversample(df):
    ...
    # 过采样逻辑
    print(f"   -> [Paper Idea] 应用过采样 ({'BorderlineSMOTE' if IMBLEARN_AVAILABLE else 'Random'})...")
    ...
    if IMBLEARN_AVAILABLE:
        ...
        X_train_res, y_train_res = resample(
            X_train_scaled[y_train == True], y_train[y_train == True],
            replace=True, n_samples=X_train_scaled[y_train == False].shape[0], random_state=42
        )
        X_train_maj = X_train_scaled[y_train == False]
        y_train_maj = y_train[y_train == False]
        X_train_res = np.vstack((X_train_res, X_train_maj))
        y_train_res = np.hstack((y_train_res, y_train_maj))
...
# --- 主流程 ---
def main():
    print("=== 基于论文理念的 SDP 框架 (过采样 + 新算法 + 智能保存) ===")
    ...
    for dataset_file in dataset_files:
        ...
        try:
            X_train, X_test, y_train, y_test = preprocess_and_oversample(df)
    ...
    results_df.to_csv("new_algorithms_results.csv", index=False)
    print(f"\n完整结果已保存到 'new_algorithms_results.csv'")
